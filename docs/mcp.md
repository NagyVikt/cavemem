...
Every live Colony MCP call has a matching top-level section in this file. Keep
this index short; the sections below carry examples, return shape notes, and
workflow guidance.

| Area | Tool | Use |
| --- | --- | --- |
| Memory | `search` | Find compact observation hits before hydrating bodies. |
| Memory | `get_observations` | Fetch selected full observation bodies by ID. |
| Memory | `timeline` | Navigate one session by compact observation IDs. |
| Memory | `list_sessions` | Find recent sessions for recall or timeline lookup. |
| Memory | `recall_session` | Audit a compact recall of another session into the current one. |
| Runtime | `hivemind` | See compact live agent/session lanes. |
| Runtime | `hivemind_context` | Combine live lanes, ownership, memory hits, and warnings before work. |
| Runtime | `attention_inbox` | See pending handoffs, unread messages, blockers, stale lanes, and claim signals. |
| Runtime | `startup_panel` | Build one compact startup card with active task, inbox, ready work, and next call. |
| Runtime | `bridge_status` | Feed OMX HUD/status with compact Colony state. |
| Tasks | `task_list` | Browse task threads; not the work picker. |
| Tasks | `task_timeline` | Read compact activity IDs for a task. |
| Tasks | `task_updates_since` | Check task changes since a timestamp, excluding your own posts. |
| Tasks | `task_post` | Add shared questions, answers, decisions, blockers, notes, and warnings. |
| Tasks | `task_note_working` | Save current working state when task resolution should be inferred. |
| Tasks | `task_claim_file` | Claim one file before editing so overlap is visible. |
| Tasks | `task_drift_check` | Compare recent edits against active claims for one task/session. |
| Tasks | `task_link` | Link related task threads. |
| Tasks | `task_links` | List related task-thread links. |
| Tasks | `task_unlink` | Remove a task-thread link. |
| Handoff | `task_hand_off` | Transfer work and optional file claims to another agent. |
| Handoff | `task_accept_handoff` | Accept a pending handoff. |
| Handoff | `task_decline_handoff` | Decline a pending handoff. |
| Handoff | `task_relay` | Emit quota/rate-limit/turn-cap resume baton when the sender may be gone. |
| Handoff | `task_accept_relay` | Accept a relay and inherit its resume recipe/claims. |
| Handoff | `task_decline_relay` | Decline a relay so it can be rerouted. |
...
| Planning | `task_plan_publish` | Publish a spec-backed plan with claimable sub-tasks. |
| Planning | `task_plan_list` | List plans and next available sub-tasks. |
| Planning | `task_ready_for_agent` | Pick or auto-claim the next available sub-task. |
| Planning | `task_plan_claim_subtask` | Claim one plan sub-task. |
| Planning | `task_plan_complete_subtask` | Complete one plan sub-task and release its claims. |
| Planning | `task_plan_status_for_spec_row` | Check whether a spec row is already covered by a plan sub-task. |
| Planning | `task_autopilot_tick` | Get one stateless next-action decision from inbox plus ready work. |
| OpenSpec | `spec_read` | Read parsed `SPEC.md` plus root hash. |
| OpenSpec | `openspec_sync_status` | Report drift between Colony task state and OpenSpec artifacts. |
| OpenSpec | `spec_change_open` | Open a spec change and task thread. |
| OpenSpec | `spec_change_add_delta` | Append one add/modify/remove delta to an open change. |
| OpenSpec | `spec_build_context` | Fetch task-scoped spec context. |
| OpenSpec | `spec_build_record_failure` | Record a build/test failure and maybe promote an invariant proposal. |
| OpenSpec | `spec_archive` | Validate, merge, and archive a spec change. |
| Examples | `examples_list` | List indexed food-source examples. |
| Examples | `examples_query` | Search indexed example patterns. |
| Examples | `examples_integrate_plan` | Build deterministic integration guidance for an example. |
| Suggestions | `task_suggest_approach` | Get similarity-backed guidance from past tasks. |
...
Build a compact startup/resume card for UIs, HUDs, or agents that need one
payload instead of manually composing `hivemind_context`, `attention_inbox`,
and `task_ready_for_agent`.
...
Returns `{ session_id, agent, repo_root, branch, active_task, ready_task,
active_queen_plan, inbox_count, blocking_items, claimed_files, blocker, next,
evidence, warnings, recommended_next_tool, recommended_next_args,
copy_paste_next_mcp_calls }`.

Use this for presentation and resume surfaces. Agents that need full
progressive-disclosure IDs should still call `hivemind_context`,
`attention_inbox`, and `task_ready_for_agent` directly.
...
Link two related task threads. Links are bidirectional and idempotent, so either
side can discover the relationship without copying decisions between threads.
...
```json
{
  ...
  "input": {
    ...
    "other_task_id": 42,
    "session_id": "sess_abc",
    "note": "frontend task depends on API contract"
  }
}
```

Returns `{ low_id, high_id, created_at, created_by, note }`. Errors with
`TASK_LINK_SELF` when both IDs are the same.

## `task_links`

List task links for one thread.

```json
{ "name": "task_links", "input": { "task_id": 17 } }
```

Returns each linked edge with the other task side, note, and link metadata.

## `task_unlink`

Remove a task-thread link after the coordination relationship is no longer
needed.

```json
{
  "name": "task_unlink",
  "input": { "task_id": 17, "other_task_id": 42 }
}
```

Returns `{ removed: true }` when a link was deleted and `{ removed: false }`
when there was no link to delete.
...
Accept quota-pending claim ownership from the linked quota handoff or relay.
This transfers all pending claims on that baton to the replacement session,
marks the baton accepted, and writes an audit note.

```json
{
  "name": "task_claim_quota_accept",
  "input": {
    "task_id": 17,
    "session_id": "sess_replacement",
    "agent": "codex",
    "handoff_observation_id": 812
  }
}
```

Use `handoff_observation_id` when the inbox gives one. Use `file_path` when a
single quota-pending file identifies the baton. Returns the updated quota claim
result from `TaskThread`.

## `task_claim_quota_decline`

Decline a quota-pending claim without cancelling the linked relay. The baton is
retargeted to any recipient so another agent can adopt it.

```json
{
  "name": "task_claim_quota_decline",
  "input": {
    "task_id": 17,
    "session_id": "sess_abc",
    "file_path": "apps/api/src/routes.ts",
    "reason": "outside this worktree"
  }
}
```

Returns the quota-decline result from `TaskThread`.

## `task_claim_quota_release_expired`

Release expired quota-pending claims from active blocker status. Matching
`handoff_pending` claims are downgraded to `weak_expired`, and expired relays or
handoffs are marked expired while audit history stays intact.

```json
{
  "name": "task_claim_quota_release_expired",
  "input": {
    "task_id": 17,
    "session_id": "sess_abc",
    "handoff_observation_id": 812
...
Pass unfinished work to another agent when quota, rate limits, turn caps, or a
manual stop mean the sender may not be available for a normal handoff.
...
```json
{
  ...
  "input": {
    ...
    "agent": "codex",
    "reason": "quota",
    "one_line": "Docs patch is half-written; finish missing relay sections.",
    "base_branch": "main",
    "to_agent": "any",
    "fetch_files_at": "abc1234",
    "expires_in_minutes": 60
  }
}
```

Relays differ from `task_hand_off`: they assume the sender is gone, weaken the
sender's claims to `handoff_pending` with a short TTL, and include a
`worktree_recipe` so the receiver can reproduce or inherit the work. If
`fetch_files_at` is omitted, the relay warns that dirty or untracked files may
need human coordination. Returns `{ relay_observation_id, status: 'pending' }`.

## `task_accept_relay`

Accept a pending relay and resume it in your session. This re-claims inherited
files under your session; inspect the relay's `worktree_recipe` before editing.

```json
{
  "name": "task_accept_relay",
  "input": { "relay_observation_id": 901, "session_id": "sess_receiver" }
}
```

Returns `{ status: 'accepted' }`. Errors include task-thread lifecycle codes
such as `OBSERVATION_NOT_ON_TASK`.

## `task_decline_relay`

Decline a relay you cannot take. The decline records a reason, cancels the
pending relay, and leaves ownership available for rerouting.

```json
{
  "name": "task_decline_relay",
  "input": {
    "relay_observation_id": 901,
    "session_id": "sess_receiver",
    "reason": "branch already pruned"
  }
}
```
...
Find a proven approach from similar past tasks before planning or
implementation. This uses embeddings when available and returns derived
guidance instead of full historical bodies.

```json
{
  "name": "task_suggest_approach",
  "input": {
    "query": "quota relay stale claims cleanup",
    "repo_root": "/abs/repo",
    "current_task_id": 17,
    "limit": 5
  }
...
Returns a suggestion payload with similar task IDs, likely files, patterns to
watch, resolution hints, and `insufficient_data_reason` when embeddings are
unavailable or no similar tasks clear the floor. Use returned IDs with
`task_timeline` or `get_observations` only when you need full history.
...
Report drift between Colony task state and durable OpenSpec artifacts. Use this
when task checkboxes, PR evidence, or cleanup records may be stale relative to
the task database.

```json
{
  "name": "openspec_sync_status",
  "input": {
    "repo_root": "/abs/repo",
...
Returns compact drift rows and exact repair actions, including stale checkboxes,
missing PR evidence, and OpenSpec cleanup mismatches.
...
Decompose a high-level goal into a Colony plan and publish it as claimable
sub-tasks. Use this when the goal is broad enough for multiple coordinated
lanes but you do not want to hand-write every sub-task.

```json
{
  "name": "queen_plan_goal",
  "input": {
    "goal_title": "Document every MCP tool",
    "problem": "Users cannot see the complete callable surface in one place.",
    "acceptance_criteria": ["Each live tool has a docs section", "Docs coverage is tested"],
    "repo_root": "/abs/repo",
    "affected_files": ["docs/mcp.md"],
    "ordering_hint": "wave",
    "finalizer": "Verify docs coverage",
    "session_id": "sess_abc",
    "dry_run": true
  }
}
...
With `dry_run: true`, returns the normalized plan without writing. Without
`dry_run`, publishes a plan with `auto_archive=true` and returns `{ plan_slug,
spec_task_id, mcp_capability_map, subtasks, plan_validation }`.

## `task_plan_validate`

Validate a hand-written multi-agent plan before publishing. It checks pairwise
file-scope overlaps, strong live claim collisions, ordered-wave errors, module
overlap warnings, and the richer plan-validation summary.
...
```json
{
  ...
  "input": {
    ...
    "subtasks": [
      {
        "title": "API",
        "description": "Build API route.",
        "file_scope": ["apps/api/src/widgets.ts"],
        "capability_hint": "api_work"
      },
      {
        "title": "UI",
        "description": "Render widget page.",
...
Returns `{ pairwise_overlaps, live_claim_collisions, module_warnings,
ordered_wave_errors, summary, partition_clean }`. A `partition_clean` value of
`false` means the plan needs sequencing, narrower file scopes, or claim
...
Check whether a root spec row is already bound to a published plan sub-task.
This prevents duplicate sub-task publication for the same `§T` or other
spec-row work item.

```json
{
  "name": "task_plan_status_for_spec_row",
  "input": { "repo_root": "/abs/repo", "spec_row_id": "T5" }
}
```

When covered, returns `{ plan_slug, subtask_index, task_id, branch,
spec_row_id, status, claimed_by_session_id, claimed_by_agent, binding }`. When
not covered, returns `{ spec_row_id, binding: null, status: null }`.
